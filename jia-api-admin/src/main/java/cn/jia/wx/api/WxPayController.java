...
public class WxPayController {
	...
	public Object parseOrderNotifyResult(@RequestBody String xmlData) throws Exception {
		try {
			...
			if(payOrderList != null && !payOrderList.isEmpty()){
				...
				PayOrderParse.instance(outTradeNo.substring(0, outTradeNo.lastIndexOf("-"))).orderNotifyResult();
			}
		} catch (WxPayException e) {
	...
	public Object parseScanPayNotifyResult(@RequestBody String xmlData, HttpServletRequest request) throws Exception {
		...
		PayOrder payOrder = PayOrderParse.instance(result.getProductId()).scanPayNotifyResult();
		payOrder.setProductId(result.getProductId());
		payOrder.setOutTradeNo(result.getProductId() + "-" + DateUtil.genTime(new Date()));
