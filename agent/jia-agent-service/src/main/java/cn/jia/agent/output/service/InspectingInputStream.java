...
final class InspectingInputStream extends FilterInputStream {
    ...
    private long count; private long nextRenew;
    ...
    @Override public int read(byte[] b,int off,int len)throws IOException{int n=super.read(b,off,len);if(n>0){count=Math.addExact(count,n);if(count>limit)throw new IOException("Upload size exceeded");digest.update(b,off,n);int keep=Math.min(n,SAMPLE_LIMIT-sample.size());if(keep>0)sample.write(b,off,keep);long now=System.currentTimeMillis();if(now>=nextRenew){renewer.renew(now);nextRenew=now+30_000L;}}return n;}
    long count(){return count;} byte[] hash(){return digest.digest();} byte[] sample(){return sample.toByteArray();}
}
