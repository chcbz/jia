...
public final class OutputUploadServiceImpl implements OutputUploadService {
    ...
    @Override public OutputUploadDTO put(String bearer,String uploadId,InputStream bytes){
        ...
        InspectingInputStream inspected=new InspectingInputStream(bytes,writer.expectedSize()+1,(now)->renew(writer,now));
        ...
        try{
            ...
            if(extra!=-1||inspected.count()!=writer.expectedSize())throw new IOException("Upload size mismatch");
        }catch(Exception failure){abandonWriter(writer);throw unavailable("OUTPUT_STORAGE_WRITE_FAILED");}
    ...
    @Override public OutputUploadDTO complete(String bearer,String uploadId,String key){
        ...
        CompleteStart start=tx.execute(s->{
            ...
            if(object.storageKey()==null||object.actualSha256()==null||upload.writerUntil()!=null)throw conflict("OUTPUT_UPLOAD_INCOMPLETE");
    ...
    private Writer beginWriter(String bearer,String uploadId){
        ...
        if(upload.writerEpoch()>0){if(scope.reservedBytes()+scope.storedBytes()+upload.expectedSize()>scope.maxBytes())throw tooLarge("OUTPUT_SCOPE_QUOTA_EXCEEDED");if(dao.changeScopeQuota(auth.tenantId(),auth.clientId(),upload.expectedSize(),0,0,now)!=1)throw tooLarge("OUTPUT_SCOPE_QUOTA_EXCEEDED");dao.abandonCleanup(auth.tenantId(),auth.clientId(),uploadId,upload.writerEpoch(),"RESERVED",upload.expectedSize(),now,now);}
    ...
    private void abandonWriter(Writer w){tx.executeWithoutResult(s->{long now=System.currentTimeMillis();dao.lockScopeQuota(w.tenant(),w.client());OutputUploadDao.UploadRow current=dao.findUpload(w.tenant(),w.client(),w.upload(),true);if(current!=null&&current.writerEpoch()==w.epoch()){dao.failWriter(w.tenant(),w.client(),w.upload(),w.epoch(),now);dao.abandonCleanup(w.tenant(),w.client(),w.upload(),w.epoch(),"RESERVED",w.expectedSize(),now,now);}});}
    ...
    private void rejectLocked(OutputUploadDao.UploadRow upload,String error,long now){if(dao.markRejected(upload.tenantId(),upload.clientId(),upload.uploadId(),upload.objectId(),error,now)!=1)return;dao.abandonCleanup(upload.tenantId(),upload.clientId(),upload.uploadId(),upload.writerEpoch(),"RESERVED",upload.expectedSize(),now,now);if(!upload.slotReleased()){dao.releaseUploadSlot(upload.tenantId(),upload.clientId(),upload.uploadId(),now);dao.changeScopeQuota(upload.tenantId(),upload.clientId(),0,0,-1,now);dao.changeBindingQuota(upload.tenantId(),upload.clientId(),upload.bindingId(),-1,now);}}
