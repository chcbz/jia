...
public class PersonalWorkspaceServiceImpl implements PersonalWorkspaceService {
    ...
    @Override public PersonalWorkspaceViews.UploadView create(Scope scope,UploadCommand command){
        ...
        try{
            ...
            PersonalWorkspaceFileEntity file=new PersonalWorkspaceFileEntity().setFileId(fileId).setOwnerJiacn(scope.ownerJiacn()).setSourceKind("UPLOAD").setDisplayName(valid.displayName()).setMediaFamily(family(valid.mime())).setState("ACTIVE").setMetadataRevision(1L).setLatestVersion(1).setCreatedAt(now).setTenantId(scope.tenantId()).setClientId(scope.clientId());
            PersonalWorkspaceVersionEntity version=new PersonalWorkspaceVersionEntity().setFileId(fileId).setOwnerJiacn(scope.ownerJiacn()).setVersion(1).setOriginalFilename(valid.filename()).setContentMimeType(valid.mime()).setByteLength(stored.byteLength()).setContentHash(stored.sha256()).setStorageUri(stored.storageUri()).setCreatedAt(now).setTenantId(scope.tenantId()).setClientId(scope.clientId());
    ...
    @Override public PersonalWorkspaceViews.PreviewView preview(Scope scope,String fileId,int version){PersonalWorkspaceVersionEntity v=versionEntity(scope,fileId,version);if("image/png".equals(v.getContentMimeType())||"image/jpeg".equals(v.getContentMimeType())||"text/plain".equals(v.getContentMimeType()))return new PersonalWorkspaceViews.PreviewView("READY",List.of(new PersonalWorkspaceViews.PreviewPart("content",v.getContentMimeType())),false,null);return new PersonalWorkspaceViews.PreviewView("UNSUPPORTED",List.of(),false,"下载查看");}
    @Override public Content readPreviewPart(Scope scope,String fileId,int version,String partId){if(!"content".equals(partId))throw new PersonalWorkspaceException(PersonalWorkspaceException.Reason.NOT_FOUND);PersonalWorkspaceViews.PreviewView p=preview(scope,fileId,version);if(!"READY".equals(p.state()))throw new PersonalWorkspaceException(PersonalWorkspaceException.Reason.UNSUPPORTED);return readContent(scope,fileId,version);}
