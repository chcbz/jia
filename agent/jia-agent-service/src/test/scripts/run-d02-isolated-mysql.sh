...
MYSQL_BASE=${D02_MYSQL_BASE:-/home/isp/apps/mysql}
MYSQLD="$MYSQL_BASE/bin/mysqld"
MYSQL="$MYSQL_BASE/bin/mysql"
MYSQLADMIN="$MYSQL_BASE/bin/mysqladmin"
EVIDENCE_DIR=${D02_EVIDENCE_DIR:-$(mktemp -d /tmp/cyf-d02-evidence-XXXXXXXX)}
BASE=$(mktemp -d /tmp/cyf-d02-mysql-XXXXXXXX)
DATA="$BASE/data"
RUN="$BASE/run"
TMP="$BASE/tmp"
SOCKET="$RUN/mysql.sock"
PIDFILE="$RUN/mysqld.pid"
GRADLE_INIT="$BASE/d02-gradle-init.gradle"
PORT=${D02_MYSQL_PORT:-$(python3 - <<'PY'
import socket
...
GRADLE_LOG="$EVIDENCE_DIR/gradle-d02-mysql.log"
JDBC_USER=d02_isolated_test
JDBC_PASSWORD=d02-isolated-only
...
printf '%s\n' \
  ...
  "}" > "$GRADLE_INIT"
printf '%s\n' "$EVIDENCE_DIR" > /tmp/cyf-d02-latest-evidence
  ...
  CLEANED=1
  {
    ...
    if [[ -S "$SOCKET" ]]; then
      schemas=$("$MYSQL" --no-defaults -uroot -S "$SOCKET" -Nse \
        "SELECT schema_name FROM information_schema.schemata WHERE schema_name LIKE 'd02\\_%' ESCAPE '\\\\'" 2>/dev/null || true)
...
flock /tmp/cyf-gradle.lock env \
  D02_MYSQL_URL="jdbc:mysql://127.0.0.1:$PORT/?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC&characterEncoding=UTF-8&connectTimeout=5000&socketTimeout=15000" \
  D02_MYSQL_USER="$JDBC_USER" \
  D02_MYSQL_PASSWORD="$JDBC_PASSWORD" \
  D02_MYSQL_EXPECTED_PORT="$PORT" \
  D02_MYSQL_EXPECTED_DATADIR="$DATA" \
  bash ./gradlew --no-daemon --max-workers=1 \
    ...
    --tests cn.jia.agent.service.impl.AgentCommandTransportMySqlTest \
    > "$GRADLE_LOG" 2>&1
gradle_status=$?
set -e

result_xml="$ROOT/agent/jia-agent-service/build/test-results/test/TEST-cn.jia.agent.service.impl.AgentCommandTransportMySqlTest.xml"
...
fixture_digest=$(agent/jia-agent-service/src/test/resources/d02/compute-fixture-digest.sh)
{
  echo "gradle_exit=$gradle_status"
  echo "gradle_selector=cn.jia.agent.service.impl.AgentCommandTransportMySqlTest"
