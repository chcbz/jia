...
MYSQL_BASE=${D07_MYSQL_BASE:-/home/isp/apps/mysql}
MYSQLD="$MYSQL_BASE/bin/mysqld"
MYSQL="$MYSQL_BASE/bin/mysql"
MYSQLADMIN="$MYSQL_BASE/bin/mysqladmin"
EVIDENCE_DIR=${D07_EVIDENCE_DIR:-$(mktemp -d /tmp/cyf-d07-evidence-XXXXXXXX)}
BASE=$(mktemp -d /tmp/cyf-d07-mysql-XXXXXXXX)
DATA="$BASE/data"
RUN="$BASE/run"
TMP="$BASE/tmp"
SOCKET="$RUN/mysql.sock"
PIDFILE="$RUN/mysqld.pid"
GRADLE_INIT="$BASE/d07-gradle-init.gradle"
PORT=${D07_MYSQL_PORT:-$(python3 - <<'PY'
import socket
...
GRADLE_LOG="$EVIDENCE_DIR/gradle-d07-mysql.log"
JDBC_USER=d07_isolated_test
JDBC_PASSWORD=d07-isolated-only
...
printf '%s\n' \
  ...
  "    if (!project.rootProject.ext.has('repoPassword')) project.rootProject.ext.repoPassword = ''" \
  "    if (!project.rootProject.ext.has('snapshotsRepoUrl')) project.rootProject.ext.snapshotsRepoUrl = 'https://repo.invalid/snapshots'" \
  "    if (!project.rootProject.ext.has('releasesRepoUrl')) project.rootProject.ext.releasesRepoUrl = 'https://repo.invalid/releases'" \
  "}" > "$GRADLE_INIT"
printf '%s\n' "$EVIDENCE_DIR" > /tmp/cyf-d07-latest-evidence
...
cleanup() {
  ...
  CLEANED=1
  {
    ...
    if [[ -S "$SOCKET" ]]; then
      schemas=$("$MYSQL" --no-defaults -uroot -S "$SOCKET" -Nse \
        "SELECT schema_name FROM information_schema.schemata WHERE schema_name LIKE 'd07\\_%' ESCAPE '\\\\'" 2>/dev/null || true)
  ...
  if [[ $(ss -ltnp '( sport = :3306 )' 2>/dev/null || true) != "$PRODUCTION_LISTENER_BEFORE" ]]; then cleanup_status=1; fi
...
[[ "$binary_version" == *"Ver 8.0.21"* ]] || {
  echo "Expected MySQL 8.0.21, got: $binary_version" >&2; exit 83;
}
...
"${RUN_AS[@]}" "$MYSQLD" --no-defaults \
  ...
  --innodb-lock-wait-timeout=5 --max-connections=40 --thread-cache-size=0 --table-open-cache=128 \
...
[[ "$version" == 8.0.21* && "$actual_port" == "$PORT" && "$actual_datadir" == "$DATA"/* ]] || {
  echo "Isolated MySQL identity mismatch: version=$version port=$actual_port datadir=$actual_datadir" >&2; exit 84;
}
...
flock /tmp/cyf-gradle.lock env \
  D07_MYSQL_URL="jdbc:mysql://127.0.0.1:$PORT/?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC&characterEncoding=UTF-8&connectTimeout=5000&socketTimeout=15000" \
  D07_MYSQL_USER="$JDBC_USER" \
  D07_MYSQL_PASSWORD="$JDBC_PASSWORD" \
  D07_MYSQL_EXPECTED_PORT="$PORT" \
  D07_MYSQL_EXPECTED_DATADIR="$DATA" \
  bash ./gradlew --no-daemon --max-workers=1 \
    ...
    --tests cn.jia.agent.service.impl.AgentCommandInboxMySqlTest \
    > "$GRADLE_LOG" 2>&1
gradle_status=$?
set -e

result_xml="$ROOT/agent/jia-agent-service/build/test-results/test/TEST-cn.jia.agent.service.impl.AgentCommandInboxMySqlTest.xml"
...
fixture_digest=$(agent/jia-agent-service/src/test/resources/d07/compute-fixture-digest.sh)
{
  echo "gradle_exit=$gradle_status"
  echo "gradle_selector=cn.jia.agent.service.impl.AgentCommandInboxMySqlTest"
  echo "gradle_lock=/tmp/cyf-gradle.lock"
  echo "gradle_max_workers=1"
  echo "gradle_jvm_xmx=384m"
