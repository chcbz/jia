...
class OutputUploadSecurityConfigurationTest {
    ...
    @Test void exactUploadRoutesRejectCookiesUserBearerAndMissingTicket()throws Exception{mvc.perform(post("/agent/output-uploads")).andExpect(status().isUnauthorized());mvc.perform(post("/agent/output-uploads").header("Authorization","Bearer user-jwt-token-value-that-is-not-run-ticket")).andExpect(status().isUnauthorized());mvc.perform(put("/agent/output-uploads/u/content").header("Authorization","Bearer terminal-ticket-value-that-is-long-enough-0000")).andExpect(status().isUnauthorized());}
