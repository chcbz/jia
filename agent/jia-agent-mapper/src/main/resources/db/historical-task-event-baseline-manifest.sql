...
DROP TEMPORARY TABLE IF EXISTS tmp_c01h_export_rows;
CREATE TEMPORARY TABLE tmp_c01h_export_rows ENGINE=InnoDB AS
...
DROP PROCEDURE IF EXISTS c01h_compute_export_digest_v1;
DELIMITER $$
CREATE PROCEDURE c01h_compute_export_digest_v1(OUT export_digest CHAR(64))
...
BEGIN
    ...
    OPEN row_cursor;
    digest_loop: LOOP
        FETCH row_cursor INTO row_digest;
        IF done = 1 THEN LEAVE digest_loop; END IF;
...
SELECT @c01h_export_manifest_digest AS manifest_digest,
       manifest_row_key, manifest_row_sha256,
       b09_report_sha256, b09_run_id, HEX(b09_operator) AS b09_operator_hex,
