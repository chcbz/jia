-- B09 permanent least-privilege routines (MySQL 8.0.21+).
-- Install only as DBA after creating the locked cyf_b09_definer@localhost
-- account and granting it the documented minimum table privileges. Operators
-- receive EXECUTE on the two atomic entry points and no direct audit/business DML.

DROP PROCEDURE IF EXISTS b09_approve_manifest_atomic_v4;
DROP PROCEDURE IF EXISTS b09_compute_staging_manifest_digest_v4;
DROP PROCEDURE IF EXISTS b09_apply_manifest_atomic_v4;
DROP PROCEDURE IF EXISTS b09_compute_approved_manifest_digest_v4;
DELIMITER $$
CREATE DEFINER=`cyf_b09_definer`@`localhost` PROCEDURE b09_compute_staging_manifest_digest_v4(
...
CREATE DEFINER=`cyf_b09_definer`@`localhost` PROCEDURE b09_approve_manifest_atomic_v4(
...
main: BEGIN
    ...
    CALL b09_compute_staging_manifest_digest_v4(computed_digest, computed_row_count);
...
CREATE DEFINER=`cyf_b09_definer`@`localhost` PROCEDURE b09_compute_approved_manifest_digest_v4(
...
CREATE DEFINER=`cyf_b09_definer`@`localhost` PROCEDURE b09_apply_manifest_atomic_v4(
...
main: BEGIN
    ...
    IF (SELECT COUNT(*)
        ...
        WHERE table_schema = DATABASE() AND table_name = 'agent_task_member'
          AND index_name = 'uk_task_member_scope') <> 4
       OR (SELECT COUNT(*)
           FROM information_schema.statistics
           WHERE table_schema = DATABASE() AND table_name = 'agent_task_member'
             AND index_name = 'uk_task_member_scope'
             AND non_unique = 0 AND sub_part IS NULL
             AND collation = 'A' AND index_type = 'BTREE' AND is_visible = 'YES'
             AND ((seq_in_index = 1 AND BINARY column_name = BINARY 'tenant_id')
               OR (seq_in_index = 2 AND BINARY column_name = BINARY 'client_id')
               OR (seq_in_index = 3 AND BINARY column_name = BINARY 'task_id')
               OR (seq_in_index = 4 AND BINARY column_name = BINARY 'agent_id'))) <> 4 THEN
    ...
    IF (SELECT COUNT(*)
        ...
        WHERE table_schema = DATABASE() AND table_name = 'agent_task_work_item'
          AND index_name = 'uk_work_item_scope') <> 3
       OR (SELECT COUNT(*)
           FROM information_schema.statistics
           WHERE table_schema = DATABASE() AND table_name = 'agent_task_work_item'
             AND index_name = 'uk_work_item_scope'
             AND non_unique = 0 AND sub_part IS NULL
             AND collation = 'A' AND index_type = 'BTREE' AND is_visible = 'YES'
             AND ((seq_in_index = 1 AND BINARY column_name = BINARY 'tenant_id')
               OR (seq_in_index = 2 AND BINARY column_name = BINARY 'client_id')
               OR (seq_in_index = 3 AND BINARY column_name = BINARY 'work_item_id'))) <> 3 THEN
    ...
    END IF;
    CALL b09_compute_approved_manifest_digest_v4(
