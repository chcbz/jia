...
public interface AgentSceneStateMapper extends BaseMapper<AgentSceneStateEntity> {
    @Select("""
                ...
                SELECT #{tenantId} AS scopeTenantId,
                       ...
                       COALESCE((
                           ...
                           WHERE scene_version_row.tenant_id = #{tenantId}
                             ...
                             AND scene_version_row.owner_jiacn = #{ownerJiacn}
                             ...
                             AND CAST(scene_version_row.owner_jiacn AS BINARY) = CAST(#{ownerJiacn} AS BINARY)
                             AND OCTET_LENGTH(scene_version_row.owner_jiacn) = OCTET_LENGTH(#{ownerJiacn})
            ...
            LEFT JOIN agent_runtime runtime
             ...
             AND CAST(runtime.owner_jiacn AS BINARY) = CAST(#{ownerJiacn} AS BINARY)
             ...
             AND EXISTS (
                 ...
                 WHERE binding.id = runtime.binding_id
                   ...
                   AND binding.jiacn = #{ownerJiacn}
                   AND CAST(binding.tenant_id AS BINARY) = CAST(#{tenantId} AS BINARY)
                   AND OCTET_LENGTH(binding.tenant_id) = OCTET_LENGTH(#{tenantId})
                   ...
                   AND CAST(binding.jiacn AS BINARY) = CAST(#{ownerJiacn} AS BINARY)
                   AND OCTET_LENGTH(binding.jiacn) = OCTET_LENGTH(#{ownerJiacn})
                   ...
                   AND identity.owner_jiacn = #{ownerJiacn}
                   ...
                   AND CAST(identity.owner_jiacn AS BINARY) = CAST(#{ownerJiacn} AS BINARY)
                   AND OCTET_LENGTH(identity.owner_jiacn) = OCTET_LENGTH(#{ownerJiacn})
                   ...
                   AND (
                       ...
                       OR EXISTS (
                           ...
                           WHERE identity_alias.registry_id = identity.id
                             ...
                             AND identity_alias.owner_jiacn = #{ownerJiacn}
                             ...
                             AND CAST(identity_alias.owner_jiacn AS BINARY) = CAST(#{ownerJiacn} AS BINARY)
                             AND OCTET_LENGTH(identity_alias.owner_jiacn) = OCTET_LENGTH(#{ownerJiacn})
            ...
            LEFT JOIN agent_scene_state state
             ...
             AND state.owner_jiacn = #{ownerJiacn}
             ...
             AND CAST(state.owner_jiacn AS BINARY) = CAST(#{ownerJiacn} AS BINARY)
             AND OCTET_LENGTH(state.owner_jiacn) = OCTET_LENGTH(#{ownerJiacn})
    ...
    @Insert("""
            INSERT INTO agent_scene_state
                (scene_id, agent_id, persona_code, behavior, origin_region_id, target_region_id,
                 ...
                 expires_at, tenant_id, client_id, owner_jiacn, create_time, update_time)
            VALUES
                (#{sceneId}, #{state.agentId}, #{state.personaCode}, #{state.behavior},
                 ...
                 #{state.expectedArrivalAt}, #{state.expiresAt}, #{tenantId}, #{clientId}, #{ownerJiacn},
