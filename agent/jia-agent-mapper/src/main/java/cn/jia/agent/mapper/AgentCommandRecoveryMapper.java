...
public interface AgentCommandRecoveryMapper {
    ...
    @Update("""
            UPDATE agent_command_delivery
            SET status='FAILED', next_retry_at=NULL, lease_owner=NULL, lease_until=NULL,
    ...
    @Update("""
            ...
            SET status='FAILED', result_status='FAILED', next_retry_at=NULL,
            ...
            WHERE id=#{inbox.id} AND version=#{inbox.version}
              ...
              AND attempt_count=#{inbox.attemptCount}
              AND delivery_id=#{inbox.deliveryId} AND event_id=#{inbox.eventId}
              AND command_id=#{inbox.commandId}
              ...
              AND ((next_retry_at=#{inbox.nextRetryAt})
                   OR (next_retry_at IS NULL AND #{inbox.nextRetryAt} IS NULL))
              AND lease_owner IS NULL AND lease_until IS NULL
              AND status=#{inbox.status} AND result_status=#{inbox.resultStatus}
              ...
              AND CAST(event_id AS BINARY)=CAST(#{inbox.eventId} AS BINARY)
              AND OCTET_LENGTH(event_id)=OCTET_LENGTH(#{inbox.eventId})
              AND CAST(command_id AS BINARY)=CAST(#{inbox.commandId} AS BINARY)
              AND OCTET_LENGTH(command_id)=OCTET_LENGTH(#{inbox.commandId})
              AND CAST(status AS BINARY)=CAST(#{inbox.status} AS BINARY)
              AND OCTET_LENGTH(status)=OCTET_LENGTH(#{inbox.status})
              AND CAST(result_status AS BINARY)=CAST(#{inbox.resultStatus} AS BINARY)
              AND OCTET_LENGTH(result_status)=OCTET_LENGTH(#{inbox.resultStatus})
            """)
    int failRecoveryInbox(
