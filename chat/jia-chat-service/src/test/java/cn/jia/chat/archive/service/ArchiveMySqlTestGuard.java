...
final class ArchiveMySqlTestGuard {
    ...
    static Target requireDisposable(Map<String, String> environment) {
        ...
        try {
            ...
            throw invalid("CYF_H02_MYSQL_URL is malformed", failure);
        }
        require(uri.getUserInfo() == null, "credentials are forbidden in CYF_H02_MYSQL_URL");
        require(uri.getQuery() == null && uri.getFragment() == null,
                "query parameters and fragments are forbidden in CYF_H02_MYSQL_URL");
        String host = uri.getHost();
        if (host != null && host.startsWith("[") && host.endsWith("]")) {
            host = host.substring(1, host.length() - 1);
        }
        require("127.0.0.1".equals(host) || "::1".equals(host),
                "CYF_H02_MYSQL_URL host must be an IP-literal loopback");
        int port = uri.getPort();
        require(port >= 1024 && port <= 65_535 && port != 3306 && port != 33060,
                "CYF_H02_MYSQL_URL requires an explicit non-production port");

        String rawPath = uri.getRawPath();
        String path = uri.getPath();
        require(path != null && path.startsWith("/") && path.indexOf('/', 1) == -1,
                "CYF_H02_MYSQL_URL must contain exactly one database name");
        require(rawPath != null && rawPath.equals(path),
                "encoded database names are forbidden");
        String databaseName = path.substring(1);
        require(databaseName.matches("cyf_h02_[a-z0-9_]{1,56}"),
                "database name must use the disposable cyf_h02_ prefix");
        require(databaseName.equals(environment.get("CYF_H02_MYSQL_DATABASE_CONFIRM")),
                "CYF_H02_MYSQL_DATABASE_CONFIRM must exactly match the URL database name");
    ...
    private static IllegalStateException invalid(String message, Throwable cause) {
        String prefix = "Unsafe H02 MySQL test target: ";
