<template>
  <div>
  <Modal
    :title="TableInfo_title"
    v-model="TableInfo_modal"
    :closable="false"
    :mask-closable="false">
    <div>
     <Form ref="TableInfo_form" :model="TableInfo_form" label-position="left" :rules="ruleValidate"  :label-width="200">
        <FormItem label="表名 (必填)" prop="name" class="11">
            <Input v-model="TableInfo_form.name"></Input>
        </FormItem>
        <FormItem label="表中文名 (必填)" prop="remark">
            <Input v-model="TableInfo_form.remark"></Input>
        </FormItem>
    </Form>
        ...
        <FormItem label="列中文名 (必填)" :rules="{validator: NotNull, trigger: 'blur'}"  :prop="'items.' + index + '.remark'">
            <Input v-model="item.remark"></Input>
             ...
             <div  class="s_icon_div">
               ...
               <li><Icon type="ios-remove-circle" size="30" @click="removeform(index)"/></li>
             </div>
            </card>
      </Form>
    ...
    <div slot="footer">
      <Button type="text" size="large" @click="TableInfo_modal=false">取消</Button>
...
export default {
  ...
  methods: {
    ...
    removeform (i) {
      if (i === 0) {
        this.$Message.error('该表单不能删除')
      } else {
        this.colFormList.items.splice(i, 1)
      }
    },
    addform () {
      this.index++
      this.colFormList.items.push(
        {
          index: this.index,
          name: '',
          type: 'int',
