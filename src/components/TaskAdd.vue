...
<div>
  <div>
    <var-picker :title="$t('task.type')" :columns="typeList" v-model="type" @change="changeType" />
    <var-picker :title="$t('task.period')" :columns="periodList" v-model="period" @change="changePeriod" />
    <var-input :label="$t('task.name')" v-model="name" />
    <var-textarea :label="$t('task.description')" v-model="description" :rows="3" />
    <var-date-picker v-model="start_time" :max-date="new Date(2100, 0, 1)" format="YYYY-MM-DD HH:mm" :minute-list="['00', '15', '30', '45']" :title="$t('task.start_time')" v-show="startTimeShow" />
    <var-date-picker v-model="end_time" :max-date="new Date(2100, 0, 1)" format="YYYY-MM-DD HH:mm" :minute-list="['00', '15', '30', '45']" :title="$t('task.end_time')" v-show="endTimeShow" />
    <var-switch :label="$t('task.lunar')" v-model="lunar" :active-value="1" :inactive-value="0" />
    <var-input :label="$t('task.amount')" v-model="amount" type="tel" :maxlength="6" v-show="amountShow" />
    <var-switch :label="$t('task.remind')" v-model="remind" :active-value="1" :inactive-value="0" />
    <br/>
